...
def part_1():
    ...
    # output_images = ['ps3-2-a-base.png']
    # input_images = ['ps3-3-c_base.jpg']
    # output_images = ['ps3-3-a-34.png']
...
def part_7():
    ...
    # homography_parameters = ps3.find_four_point_transform(points1,points2)
...
def part_9_b(path1,path2):
    ...
    im_dst = cv2.imread(path2, 1)
    out_img=im_src.copy()
    src_gray=cv2.cvtColor(im_src, cv2.COLOR_BGR2GRAY)
    dst_gray=cv2.cvtColor(im_dst, cv2.COLOR_BGR2GRAY)
    A_point,B_point=ps3.detectAndDescribe(dst_gray, src_gray)
    homo=ps3.ransac(A_point,B_point)
    result = ps3.project_imageA_onto_imageB(im_dst, im_src, homo, 1)
    result[0:out_img.shape[0], 0:out_img.shape[1]] = out_img

    # cv2.imshow("Warped Source Image", result)
    cv2.imwrite(IMG_DIR + '\ps3-9-2.jpg', result)
    cv2.waitKey(0)
