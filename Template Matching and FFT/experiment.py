...
def template_match_test():
    ...
    for img_fl, img_template_fl, label in zip(
        ...
        for method in ("tm_ssd", "tm_nssd", "tm_ccor", "tm_nccor"):
            ...
            bottom_right = (img_template.shape[:2][1]+top_left[0],img_template.shape[:2][0]+top_left[1])
...
def compression_runner():
    ...
    img_compressed, compressed_frequency_img = ps2.compress_image_fft(
        img_bgr, keep)
    cv2.imwrite(OUTPUT_DIR + 'ps2-4-a-3.jpg', img_compressed)
    cv2.imwrite(OUTPUT_DIR + 'ps2-4-a-3-freq.jpg',
...
def low_pass_filter_runner():
    ...
    cv2.imwrite(OUTPUT_DIR + 'ps2-5-a-3.jpg', img_low_pass)
    cv2.imwrite(OUTPUT_DIR + 'ps2-5-a-3-freq.jpg',
