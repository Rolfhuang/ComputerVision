...
def visualize_mean_face(x_mean, size, new_dims):
    ...
    normalized_image = cv2.normalize(mean_face_2d, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    resized_img = cv2.resize(normalized_image, new_dims)
    return resized_img
...
def part_2a():
    ...
    rand_accuracy = float(100 * sum(ytrain == rand_y)/ Xtrain.shape[0])
    ...
    # TODO: find which of these labels match ytrain and report its accuracy
    wk_accuracy = float(100 * sum(ytrain == wk_results)/ Xtrain.shape[0])
    # raise NotImplementedError
    print('(Weak) Training accuracy {0:.2f}%'.format(wk_accuracy))

    num_iter = 5
    for i in range(1,num_iter+1):
        Xtrain_b, ytrain_b, Xtest_b, ytest_b = ps6.split_dataset(X, y, p)
        boost = ps6.Boosting(Xtrain_b, ytrain_b, i)
        boost.train()
        good, bad = boost.evaluate()
        boost_accuracy = 100 * float(good) / (good + bad)
        print('(Boosting) Training accuracy' +str(i) +' {0:.2f}%'.format(boost_accuracy))

    # Picking random numbers
    rand_y = np.random.choice([-1, 1], (len(ytest)))
    # TODO: find which of these labels match ytest and report its accuracy
    rand_accuracy = float(100 * sum(ytest == rand_y)/ Xtrain.shape[0])
    # raise NotImplementedError
    ...
    # TODO: find which of these labels match ytest and report its accuracy
    wk_accuracy = float(100 * sum(ytest == wk_results)/ Xtrain.shape[0])
    # raise NotImplementedError
    print('(Weak) Testing accuracy {0:.2f}%'.format(wk_accuracy))

    y_pred = boost.predict(Xtest)
    # TODO: find which of these labels match ytest and report its accuracy
    boost_accuracy = float(100 * sum(ytest == y_pred)/ Xtrain.shape[0])
    # raise NotImplementedError
...
def part_4_a_b():
    ...
    for i in range(len(labels)):
        if predictions[i] == labels[i]:
            correct += 1
    vj_accuracy = (float(correct) / len(labels)) * 100
    # raise NotImplementedError
    ...
    vj_accuracy = (float(correct) / len(real_labels)) * 100
